...
print ("pret")
with open("/sd/BurstyBezier_calc.thr", "r") as f1:
    init = 0
    theta0 = 0
    phi0 = 0
    ligne=0
    ...
    for line in f1:
        ...
        if "." in line[0:15] and len(line) > 1 and not "#" in line[0:5]:
            ...
            ligne = ligne + 1
            theta = 1E4*float(line.split()[0])
            Ray = float(line.split()[1])
            print("ligne :",ligne," theta :",theta/1E4," Ray :",Ray,"Ray0 :",Ray0)
            Phi = math.acos((Ray*Ray*4-2)/(2))
            thetac = 1E4*math.acos(Ray) - theta 
            ...
            if init == 1 :
                ...
                direction2.low()
                bm1 = (theta - theta0)/1E4 * (38400 / math.pi / 2) 
                totalbm1 = totalbm1 + bm1
                #bm1 = bm1 #+ restbm1
